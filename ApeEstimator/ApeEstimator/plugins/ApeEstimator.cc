...
class ApeEstimator : public edm::EDAnalyzer {
   ...
   private:
	...
	float errY2;
      ...
      //TrackStruct::HitParameterStruct fillHitVariables(const GeomDet&,  const TrajectoryMeasurement&, const edm::EventSetup&);
...
//std::cout<<"isPixel   "<<isPixel<<" isStrip  "<<isStrip<<std::endl;
...
//ApeEstimator::fillHitVariables(const GeomDet& geomDet, const TrajectoryMeasurement& i_meas, const edm::EventSetup& iSetup){
...
/*   edm::ESHandle<TrackerGeometry> tkGeom;
   ...
   tkGeom = trackerBuilder.build(&*geometricDet, parameterSet_);
...
*/
//    bool isPixel = dynamic_cast<const PixelGeomDetUnit&>(detUnit)!=0;
//    bool isStrip = dynamic_cast<const StripGeomDetUnit&>(detUnit)!=0;
...
/*
    ...
    for(std::vector<unsigned int>::const_iterator i_rawId = tkSector.v_rawId.begin();
      ...
      else if(m_tkTreeVar_[*i_rawId].subdetId==StripSubdetector::TIB || m_tkTreeVar_[*i_rawId].subdetId==StripSubdetector::TOB ||
         m_tkTreeVar_[*i_rawId].subdetId==StripSubdetector::TID || m_tkTreeVar_[*i_rawId].subdetId==StripSubdetector::TEC){
        Strip = true;
      }
      else { edm::LogWarning("FillHitVariables")<<"cant identify wether it is from pixel or strip";
        hitParams.hitState = TrackStruct::invalid; return hitParams;}

    

std::cout<<"pixel   "<<Pixel <<"strip   "<<Strip<<std::endl;
}
*/
   ...
   if(!hit.detUnit()){hitParams.hitState = TrackStruct::invalid; return hitParams;} // is it a single physical module?
    ...
    const PixelGeomDetUnit * pixelDet = (const PixelGeomDetUnit*)(&detUnit);
     const LocalError& lape = pixelDet->localAlignmentError();
           if (lape.valid())
              ...
              //std::cout<<" from pixel xx "<<lape.xx()<<" from pixel   xy "<<lape.xy()<<" from pixel  yy "<<lape.yy()<<std::endl;
    ...
    if(!dynamic_cast<const StripTopology*>(&detUnit.type().topology())){hitParams.hitState = TrackStruct::invalid; return hitParams;}
     ...
     const LocalError& lape = stripDet->localAlignmentError();
           if (lape.valid())
              { errorWithoutAPE = LocalError(errHitApe.xx() -lape.xx(), errHitApe.xy()- lape.xy(), errHitApe.yy()-lape.yy());
              //std::cout<<" from strip xx"<<lape.xx()<<"strip   xy "<<lape.xy()<<" strip yy "<<lape.yy()<<std::endl;
		}
	}
...
//	if (lape.valid())
  ...
  const StatePositionAndError2 positionAndError2HitWoApe = this->positionAndError2(lPHit, errHitWoApe, hit);
	...
	//std::cout<<"xx"<<errHitApe.xx()<<"   xy "<<errHitApe.xy()<<"  yy "<<errHitApe.yy()<<std::endl;
        //std::cout<<"xx"<<lape.xx()<<"   xy "<<lape.xy()<<"  yy "<<lape.yy()<<std::endl;
